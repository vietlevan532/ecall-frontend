...
function SignUpSaleAccount() {
  ...
  const [isMissingRepassword, setIsMissingRepassword] = useState<boolean>(false);
  ...
  const registerSale = (): void => {
    ...
    fetch("/api/admin/sale-register", requestOptions)
      .then((response) => {
  ...
  const submit = (): void => {
    ...
    if (isPasswordValid) {
      ...
      alert('Tạo tài khoản nhân viên không thành công');
    }
  };
...
export default SignUpSaleAccount;
